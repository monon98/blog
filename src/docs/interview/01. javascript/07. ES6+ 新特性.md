...
2. **不支持的操作**：
   - BigInt 不能使用一元加号操作符（+）
   - BigInt 支持大部分位运算符（`&`、`|`、`^`、`~`、`<<`、`>>`），但不能与 Number 混合使用
   - BigInt 不支持无符号右移（`>>>`）
...
1. **Promise.withResolvers()**：提供一种更简洁的方式来创建和控制 Promise。
...
2. **Object.groupBy() 和 Map.groupBy()**：对数据进行分组聚合。
...
3. **String.prototype.isWellFormed() 和 toWellFormed()**：检查和修复字符串中的 Unicode 代理对。
...
4. **RegExp `/v` 增强标志**：支持更高级的字符集操作。

```javascript
// 使用 /v 标志进行集合操作
const regex = /[\p{L}&&\p{Latin}]/v;
console.log(regex.test("a")); // true
console.log(regex.test("あ")); // false
```

5. **ArrayBuffer 可调整大小和可转移**：支持调整 ArrayBuffer 大小和跨线程转移。
...
const transferable = new ArrayBuffer(1024);
worker.postMessage(transferable, [transferable]);
console.log(transferable.byteLength); // 0（已转移）
```

### ES2025（ES16）

1. **Iterator Helpers**：为迭代器提供 map、filter、reduce 等方法。
...
2. **Set 方法扩展**：为 Set 添加 union、intersection、difference 等方法。
...
3. **Promise.try()**：统一处理同步和异步函数的 Promise 包装。

```javascript
// 之前的写法
async function fetchData() {
  try {
    const result = await Promise.resolve(mightThrow());
    return result;
  } catch (e) {
    // 错误处理
  }
...
4. **RegExp.escape()**：转义字符串中的正则表达式特殊字符。

```javascript
const user input = "Hello World (price: $5.00)";
const escaped = RegExp.escape(userInput);
const regex = new RegExp(escaped);
// 可以安全地匹配用户输入
```

5. **正则表达式模式修饰符**：在正则表达式内部启用/禁用标志。
...
7. **Float16Array**：新增 16 位浮点数类型，适用于图形和机器学习。
