---
created: 2025-12-16T18:12:27 (UTC +08:00)
tags: [前端,后端,Nginx中文技术社区,前端开发社区,前端技术交流,前端框架教程,JavaScript 学习资源,CSS 技巧与最佳实践,HTML5 最新动态,前端工程师职业发展,开源前端项目,前端技术趋势]
source: https://juejin.cn/post/7582156410320371722
author: LiuMingXin
---
...
---
![image.png](https://p6-xtjj-sign.byteimg.com/tos-cn-i-73owjymdk6/f9ca040f0463416ca5a104ef0e104549~tplv-73owjymdk6-jj-mark-v1:0:0:0:0:5o6Y6YeR5oqA5pyv56S-5Yy6IEAgTGl1TWluZ1hpbg==:q75.awebp?rk3s=f64ab15b&x-expires=1766474392&x-signature=zJQllzew7zaoeOS7N9ATUNGVeuI%3D) **下面是我在nginx使用过程中发现的几个问题，分享出来大家一起熟悉一下nginx**
...
-   [http://192.186.0.1:8080/test/file/getList](https://link.juejin.cn/?target=http%3A%2F%2F192.186.0.1%3A8080%2Ftest%2Ffile%2FgetList "http://192.186.0.1:8080/test/file/getList") （配置一的结果）
-   [http://192.186.0.1:8080/file/getList](https://link.juejin.cn/?target=http%3A%2F%2F192.186.0.1%3A8080%2Ffile%2FgetList "http://192.186.0.1:8080/file/getList") （配置二的结果）
...
> 因为我的服务器可能是不同的架构，比如 x86\_64, ARM等等
...
**当有一天你使用unity 3d开发应用并导出`wasm项目`后，需要使用nginx部署后，当你和往常一样正常部署后，一访问发现报错误！**
...
进入`nginx`的安装目录，找到`mine.types`文件，新增下面的配置：
...
下面是`wasm应用`的配置示例，是可以直接使用的，只需要的修改一下`访问文件的路径`和`端口`即可。
...
2.  **Nginx.conf 核心配置**：
    ...
    -   **静态资源缓存**：为 `StreamingAssets`、`Build`、`TemplateData` 及图片/CSS 设置合理的缓存策略（Cache-Control）。
    -   **HTML 更新策略**：HTML 文件应禁用缓存（`no-cache`），确保用户始终加载最新版本。
    -   **性能优化**：开启 Gzip 压缩，提高传输效率。
    -   **安全加固**：添加基本的安全头配置，保护服务器资源。

文章同步地址：[www.liumingxin.site/blog/detail…](https://link.juejin.cn/?target=https%3A%2F%2Fwww.liumingxin.site%2Fblog%2Fdetail%2Fnginx "https://www.liumingxin.site/blog/detail/nginx")
